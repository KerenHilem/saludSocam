...
<section> 
  <div class="container">
       <div class="col-md-12">
        <h2>Citologias Retornadas Por La Unidad</h1>
      </div>
  ...
  <div class="table-responsive col-md-12">
   <table class="table table-hover">
      ...
      <tbody>
        ...
        $no = 1;
        if ($_SESSION['nivel']==1) {
         $query = $mysqli->query("SELECT DISTINCT citologia.id,citologia.creado, citologia.motivo_repeticion,pacientes.apellido_pat, pacientes.apellido_mat, pacientes.nombre, citologia.numero_citologico, unidades.nombre AS unidades_nombre FROM pacientes, unidades, citologia WHERE pacientes.id=citologia.paciente_id AND unidades.id=citologia.unidad_id AND citologia.estatus='ENVIO DE MUESTRAS REPETIDAS A JURISDICCION'");
        }else if ($_SESSION['nivel']==2) {
        $query = $mysqli->query("SELECT DISTINCT citologia.id,citologia.creado, citologia.motivo_repeticion,pacientes.apellido_pat, pacientes.apellido_mat, pacientes.nombre, citologia.numero_citologico, unidades.nombre AS unidades_nombre FROM pacientes, unidades, citologia WHERE pacientes.id=citologia.paciente_id AND unidades.id=citologia.unidad_id AND citologia.estatus='ENVIO DE MUESTRAS REPETIDAS A JURISDICCION' AND unidades.jurisdiccion_id='$jurisdiccion'");
      }
